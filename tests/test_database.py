...
async def test_create_tracker(db):
    tid = await db.create_tracker(chat_id=100, name="standup", user_id=1)
    assert tid is not None

    # Duplicate name in same chat fails
    dup = await db.create_tracker(chat_id=100, name="standup", user_id=2)
    assert dup is None

    # Same name in different chat succeeds
    tid2 = await db.create_tracker(chat_id=200, name="standup", user_id=1)
    assert tid2 is not None


@pytest.mark.asyncio
async def test_get_and_list_trackers(db):
    await db.create_tracker(100, "alpha", 1)
    await db.create_tracker(100, "beta", 1)

    t = await db.get_tracker(100, "alpha")
    assert t is not None
    assert t["name"] == "alpha"

    trackers = await db.list_trackers(100)
    assert len(trackers) == 2

    assert await db.get_tracker(100, "nonexistent") is None


@pytest.mark.asyncio
async def test_delete_tracker(db):
    tid = await db.create_tracker(100, "temp", 1)
    await db.delete_tracker(tid)
    assert await db.get_tracker(100, "temp") is None
...
async def test_add_and_remove_participant(db):
    ...
    assert await db.remove_participant(tid, 10) is False  # Already removed
...
@pytest.mark.asyncio
async def test_name_case_insensitive(db):
    tid = await db.create_tracker(100, "Standup", 1)
    t = await db.get_tracker(100, "standup")
    assert t is not None
    assert t["id"] == tid
